"""
demographics Excel 의 연구번호·나이 로 testdataset.csv 를 60세 기준 분할 저장.

- 나이 < 60  -> testdataset_age60_under.csv
- 나이 >= 60 -> testdataset_age60_upper.csv
"""
...
OUT_UNDER = "testdataset_age60_under.csv"
OUT_UPPER = "testdataset_age60_upper.csv"


def _find_columns(df: pd.DataFrame) -> tuple[str, str]:
    """연구번호·나이 컬럼명 탐색 (한글 헤더 또는 위치 기반)."""
    id_col = age_col = None
    for col in df.columns:
        name = str(col).strip()
        if "연구번호" in name or name.lower() in ("id", "subject_id", "research_id"):
            id_col = col
        if "나이" in name or name.lower() in ("age", "ages"):
            age_col = col
    if id_col is None and len(df.columns) >= 2:
        id_col = df.columns[1]
    if age_col is None and len(df.columns) >= 5:
        age_col = df.columns[4]
    if id_col is None or age_col is None:
        raise ValueError(f"연구번호/나이 컬럼을 찾을 수 없습니다: {list(df.columns)}")
    return id_col, age_col


def split_testdataset_by_age(
    demo_path: str,
    test_path: str,
    ...
    id_col, age_col = _find_columns(demo)
    demo = demo[[id_col, age_col]].copy()
    demo.columns = ["id", "age"]
    demo["id"] = demo["id"].astype(str).str.strip()
    demo["age"] = pd.to_numeric(demo["age"], errors="coerce")
    demo = demo.dropna(subset=["id", "age"]).drop_duplicates(subset=["id"], keep="first")
    ...
    if missing:
        print(f"WARNING: 나이 매칭 실패 id ({len(missing)}): {missing}")

    under = merged[merged["age"] < age_threshold].drop(columns=["age"])
    upper = merged[merged["age"] >= age_threshold].drop(columns=["age"])
...
def main():
    ...
    parser.add_argument("--threshold", type=int, default=AGE_THRESHOLD)
    ...
    info = split_testdataset_by_age(
        args.demo, args.test, args.out_under, args.out_upper, args.threshold
    )
    print(f"testdataset rows : {info['test_total']}")
    print(f"matched with age : {info['matched']}")
    print(f"age < {args.threshold}  -> {info['out_under']} ({info['under_n']} rows)")
    print(f"age >= {args.threshold} -> {info['out_upper']} ({info['upper_n']} rows)")
